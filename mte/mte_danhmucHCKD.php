...
class MySQLtabledit {
	...
	####################
		...
		if ($_GET['sort'] && in_array($_GET['sort'],$this->fields_in_list_view) ) {
			...
			$order_by = "ORDER by $this->primary_key ASC";	
		}
		...
		$sql="SELECT `stt`,`tenviettat`,`tenthietbi`,`somay`,`hangsx`,`bophansh`,`thoihankd`,`chusohuu`,date_format(`ngayktnghiemthu`,'%d-%m-%Y') as ngayktnghiemthu,left(somay,2) as tlkt FROM `thietbihckd_iso` $where_search $order_by";
	...
	##################
		...
		$in_id = $_GET['id'];
        $in_sql=mysql_query("SELECT `tenviettat`,`somay` FROM `thietbihckd_iso` WHERE `stt`='$in_id'");
	...
	###################
		...
		/// STT
			$recc=0;
		$r3=mysql_query("SELECT max(stt) as tt FROM thietbihckd_iso");
		...
		if($key=="loaitb") {
			if($value==""){
			 $field = "<select name='$key' $style $field_id>
			  <option value=''></option>
			  <option value='1'> Thiết bị theo dõi và đo lường</option>
			  <option value='2'> Máy bắn mìn</option>
			  <option value='3'> Máy kiểm tra kíp mìn </option>
			  <option value='4'> Máy đo độ lệch </option>
			  <option value='5'> Mẫu chuẩn,vật chuẩn </option>
			  <option value='6'> Thiết bị đo lường chuyên dụng </option>
			 </select>";
			}elseif($value=="1"){
		     $field = "<select name='$key' $style $field_id>
			  <option value='1'> Thiết bị theo dõi và đo lường</option>
		  ...
		  if($key=="ngayktnghiemthu"){
				if($value!="0000-00-00 00:00:00"){
			   		 $value=date("Y-m-d", strtotime($value));
			    	 $field = "<input type=date name='$key' value='$value'>";
				}else{
					 $field = "<input type=date name='$key'>";
				}
			}
			
            if($show_key!="active" && $key!="mavattu")   // Dong muon an di
            {
		...
		$this->content = "
				...
				<div style='width: $this->width_editor;background:#454545'>
					...
					<table cellspacing=0 cellpadding=0 style='border: 0px solid white'>
						<tr>
						<td>
							<button onclick='window.location=\"danhmucHCKD.php\";' style='margin: 20px 15px 25px 15px; border: 1px solid #000;'>{$this->text['Go_back']}</button></td>
						<td>
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	###################
        ...
        foreach($_POST AS $key => $value) {
            ...
            if ($key=="somay")
            {
		    $in_somay=$value;             // GIU LAI MASOMAY DE THONG BAO SAU KHI UPDATE HOAC EDIT
	    }
            if ($key=="tenviettat"){
		  ...
		  $mavattu="$in_TenThietBi-$in_somay";
			
			
		  if($key=="tlkt") {
		  $max_file_size = 1024*1000*10; //10M  
		  if (!file_exists("TLKT/$in_TenThietBi/")) mkdir("TLKT/$in_TenThietBi/");
		  $folder="TLKT/$in_TenThietBi/";
		...
		$check=0;
            ...
            while ($rij = mysql_fetch_array($result))
			{
				if(($in_TenThietBi==$rij['tenviettat'])&&($in_somay==$rij['somay']))	
						$check=1;
			} 
	    ...
	    if ($in_mte_new_rec) {
			if($check==0){
				$sql =  "INSERT INTO `$this->table`(mavattu,$insert_fields) VALUES ('$mavattu',$insert_values);";	
            	}
		}
		# edit record:
		else {
