...
class MySQLtabledit {
	...
	####################
		...
		if($_GET['f']=='thongke'){
			$where_search = "WHERE loaidau!='' ";
		}
		$sql="SELECT `stt`, `somay`,`mavt`,`tenvt`,`mamay`,`madv`,`bdtime`,date_format(`ngayktsd`,'%d-%m-%Y') as ngayktsd,`loaidau`,`mucdau`,left(tenvt,2) as tlkt,left(tenvt,2) as hosomay  FROM `thietbi_iso` $where_search $order_by";
		...
		if (mysql_num_rows($result)>0) {
			...
			while ($rij = mysql_fetch_assoc($result)) {
				...
				foreach ($rij AS $key => $value) {
				    ...
				    if (($key=="tlkt")||($key=="hosomay")||($key=="thongtincb")) 
				    ...
				    elseif ($key=="loaidau")
					    $head .= "<td NOWRAP style='border:0px solid white;border-bottom:1px solid black'><a href='$this->url_script?$query_sort' class='mte_head'>Loại dầu</a> $sort_image</td>";
				    else
						...
						if ($key == $this->primary_key) {
							$buttons = "<td NOWRAP><a href='javascript:void(0)' onclick='del_confirm($value)' title='Delete {$this->show_text[$key]} $value'><div class=\"noprint\"><IMG SRC='$this->url_base/images/del.png' WIDTH=16 HEIGHT=16 BORDER=0 ALT=''></a>
							...
							if($key=="mavt") $mavt=$value;
							if($key=="somay") $somay=$value;
							if($key=="ngayktsd") if ($value=="00-00-0000") $value="";
							if($key=="hosomay"){
							 	
								$this_row .="<td style='border:0px solid white;border-bottom:1px solid black'><a  href='download.php?mavthosomay=$mavt/$somay'  title='HOSM' ><span style=\"color:blue;\"><div class=\"noprint\">$mavt/$somay</div></span></a></td>";
							}elseif($key=="tlkt"){
							...
							}elseif($key=="thongtincb") {
								$thongtincb=$value;

								$this_row .= "<td style='border:0px solid white;border-bottom:1px solid black'><div class=\"noprint\"> <textarea readonly style=\"border:none;background:$background\">" . substr(strip_tags($value), 0, 500) . '</textarea></div></td>';
							}elseif($key=="loaidau") {
                                                                 
								$this_row .= "<td style='border:0px solid white;border-bottom:1px solid black'>" . substr(strip_tags($value), 0, 500) . '</td>';
							}else
								$this_row .= '<td>' . substr(strip_tags($value), 0, 500) . '</td>';
						}
					}
				}
	...
	###################
		...
		/// STT
		$recc=0;
		$r3=mysql_query("SELECT max(stt) as tt FROM thietbi_iso");
...
//////		
		...
		foreach ($rij AS $key => $value) {
            ...
            if($key=="madv"){
                ...
                while($row = mysql_fetch_array($in_result))
                    {
                        if($row[madv]==$value)
                            $options .= "<option selected value='$row[madv]'>$row[tendv]-$row[madv]</option>"; 
                        else
                            $options .= "<option value='$row[madv]'>$row[tendv]-$row[madv]</option>"; 
	    ...
	    if ($show_key=="Mã Hiệu") $mavt=$value; 
	    if ($key=="mamay") { 
		    $field="<input type=\"hidden\" name=\"$key\" value=\"\" />";
		    $show_key="";
	    }
	    if ($show_key=="TLKT") {
		    $field="
			    <input type=\"file\" id=\"file\" name=\"tlkt[]\" multiple=\"multiple\"/><input type=\"hidden\" name=\"tlkt\" value=\"\" /></br>
			    <input type=\"file\" id=\"file\" name=\"tlkt[]\" multiple=\"multiple\"/></br>
			    <input type=\"file\" id=\"file\" name=\"tlkt[]\" multiple=\"multiple\"/></br>
			    <input type=\"file\" id=\"file\" name=\"tlkt[]\" multiple=\"multiple\"/></br>
			    <input type=\"file\" id=\"file\" name=\"tlkt[]\" multiple=\"multiple\"/></br>
			    ";
	    }
	    if ($show_key=="Hồ sơ máy") {
		    $field="<input type=\"file\" id=\"file\" name=\"hosomay\" /><input type=\"hidden\" name=\"hosomay\" value=\"\" />";
	    }
            if($key=="bdtime")
	    { 
		   if ($value!="") $hankd=$value;
		   else $hankd=12;
		   $field = "<input type='text' name='$key' value='$hankd' maxlength='{$matches[1]}' $style $readonly $field_id>";  
	    } 
	    if($key=="ngayktsd")
	    { 
		    if (($value=="")||($value=="0000-00-00")) $value="0000-00-00";
		    else $value = date("d/m/Y",strtotime("$value"));  
		   $field = "<input name='ngayktsd'  type='text' value=\"$value\">
				       <A HREF=\"#\" onClick=\"cal.select(document.forms['example'].ngayktsd,'anchor1','dd/MM/yyyy'); return false;\"
		...
		$this->content = "
				...
				<div style='width: $this->width_editor;background:#454545'>
					...
					<table cellspacing=0 cellpadding=0 style='border: 0px solid white'>
						<tr>
						<td>
							<button onclick='window.location=\"Danhmucsc.php\";' style='margin: 20px 15px 25px 15px; border: 1px solid #000;'>{$this->text['Go_back']}</button></td>
						<td>
				...
				<div style='width: $this->width_editor;background:#454545'>
					...
					<table cellspacing=0 cellpadding=0 style='border: 0px solid white'>
						...
						<td>
							<button onclick='window.location=\"{$_SESSION['hist_page']}\";' style='margin: 20px 15px 25px 15px; border: 1px solid #000;'>{$this->text['Go_back']}</button></td>
						<td>
							<form enctype=\"multipart/form-data\" method=post action='$this->url_script' onsubmit='return submitform()'>
							<input type='submit' value='{$this->text['Save']}' style='width: 80px;border: 1px solid #000; margin: 20px 0 25px 0'></td>
						</tr>
					</table>
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	###################
		...
		$value="$nams-$thangs-$ngays";
            ...
            {
		    if($model!="")
			    $value="$in_mavt-$model-$in_somay";
		    else
			    $value="$in_mavt-$in_somay";
            }
	    if ($key=="tlkt") {
		  $max_file_size = 1024*1000*10; //10M  
		  if (!file_exists("TLKT/$in_mavt/")) mkdir("TLKT/$in_mavt/");
		  $folder="TLKT/$in_mavt/";
		  ...
		  //if ($target_file!=$folder) {
				 ...
				 */
					if(!empty($_FILES['tlkt']['name'])){
					// Loop $_FILES to exeicute all files
					foreach ($_FILES['tlkt']['name'] as $f => $name) {     
					    if ($_FILES['tlkt']['error'][$f] == 4) {
						continue; // Skip file if any error found
					    }	       
					    if ($_FILES['tlkt']['error'][$f] == 0) {	           
						if ($_FILES['tlkt']['size'][$f] > $max_file_size) {
						    $message[] = "$name is too large!.";
						    continue; // Skip large files
						}
						else{ // No error found! Move uploaded files
							if(move_uploaded_file($_FILES["tlkt"]["tmp_name"][$f], $folder.$name))
							    echo "The file ". basename( $_FILES["tlkt"]["name"]). " has been uploaded.";
						     	else 
							    echo "Sorry, there was an error uploading your file.";
						    }
	    ...
	    if ($key=="hosomay") {
		  if (!file_exists("HOSM/$in_mavt/")) mkdir("HOSM/$in_mavt/");
		  if (!file_exists("HOSM/$in_mavt/$in_somay/")) mkdir("HOSM/$in_mavt/$in_somay/");
		  $folder="HOSM/$in_mavt/$in_somay/";

		  $target_file = $folder . basename($_FILES["hosomay"]["name"]);
		  if ($target_file!=$folder) {
						...
						// Check file size
					/*	if ($_FILES["hosomay"]["size"] > 10000000) {
						    echo "Sorry, your file is too large.";
						    $uploadOk = 0;
					}
					 */
						...
						// if everything is ok, try to upload file
						    ...
						    if (move_uploaded_file($_FILES["hosomay"]["tmp_name"], $target_file)) {
							echo "The file ". basename( $_FILES["hosomay"]["name"]). " has been uploaded.";
						    } else {
							    echo "Sorry, there was an error uploading your file.";
						    }
						}
		...
		$check=0;
			$result=mysql_query("SELECT * FROM `$this->table`");
            		while ($rij = mysql_fetch_array($result))
			{
				if(($in_mavt== $rij['mavt'])&&($temp==$rij['somay'])&&($model==$rij['model']))	
						$check=1;
			} 
...
<input type=hidden name=\"submit\" value=\"Danh mục thiết bị\">
