...
class MySQLtabledit {
	...
	####################
        ...
        $sql="SELECT `stt`,`mavitri`,`tenvitri` FROM `vitri_iso` $where_search $order_by";
			...
			while ($rij = mysql_fetch_assoc($result)) {
				...
				foreach ($rij AS $key => $value) {
					...
					if (in_array($key, $this->fields_in_list_view)) {
						...
						if ($key == $this->primary_key) {
							$buttons = "<td NOWRAP ><div class=\"noprint\"><a href='javascript:void(0)' onclick='del_confirm($value)' title='Delete {$this->show_text[$key]} $value'><IMG SRC='$this->url_base/images/del.png' WIDTH=16 HEIGHT=16 BORDER=0 ALT=''></a>
	...
	##################
		...
		$in_id = $_GET['id'];
        $in_sql=mysql_query("SELECT `mavitri`,`tenvitri` FROM `vitri_iso` WHERE `stt`='$in_id'");
	...
	###################
		...
		/// STT
			$recc=0;
		$r3=mysql_query("SELECT max(stt) as tt FROM vitri_iso");
...
//////		
	    ...
	    {  
		   $field = "<input type='text' name='$key' value='12' maxlength='{$matches[1]}' $style $readonly $field_id>"; 
                } 
            if($show_key=="Check nếu là thiết bị chuyên dụng của Xưởng")
	    {  
		    if ($value=='1')
			    $field = "<input type='checkbox' name='$key' value='1' maxlength='{$matches[1]}' $style $readonly $field_id checked>"; 
		    else
			    $field = "<input type='checkbox' name='$key' value='1' maxlength='{$matches[1]}' $style $readonly $field_id>"; 
                } 
            if($show_key=="Ngày KD/BD")
	    {  
		   $field = "<input name='ngaykd'  type='text' value=\"$value\">
				       <A HREF=\"#\" onClick=\"cal.select(document.forms['example'].ngaykd,'anchor1','yyyy-MM-dd'); return false;\"
		...
		$this->content = "
				...
				<div style='width: $this->width_editor;background:#454545'>
					...
					<table cellspacing=0 cellpadding=0 style='border: 0px solid white'>
						<tr>
						<td>
							<button onclick='window.location=\"vitri.php\";' style='margin: 20px 15px 25px 15px; border: 1px solid #000;'>{$this->text['Go_back']}</button></td>
						<td>
		...
		# edit record:
			...
			$sql = "UPDATE `$this->table` SET $updates WHERE $where LIMIT 1; ";
                ...
                if (mysql_query($sql)) {
        			...
        			$_SESSION['content_saved'] = "
        				<div style='width: $this->width_editor'>
        					<div style='padding: 10px; color:#fff; background: #67B915; font-weight: bold'>{$this->text['saved']} </br>vị trí $temp </div>
        				</div>
        				";
        			if ($in_mte_new_rec) {
