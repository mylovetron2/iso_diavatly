       ...
       # message after add or edit
		...
		if ($_GET['s'] && $_GET['f']) {
			...
			if ($in_search_field == $this->primary_key) {
				...
				$where_search = " and $in_search_field LIKE '%$in_search%' and ttktafter !='Hỏng'";
			}
		}
		...
		// build query_string
		// query_joomla_component (joomla) 
		if ($this->query_joomla_component1) $query_string1 = '&option=' . $this->query_joomla_component1 ;
		// navigation
		$query_string1 .= '&start=' . $start;
		// sorting
		$query_string1 .= '&ad=' . $_GET['ad']  . '&sort=' . $_GET['sort'] ;
		// searching
		$query_string1 .= '&n=' . $_GET['n']  . '&v=' . $_GET['v'] ;
		$y = date('Y');
		$in_sql="SELECT  distinct `mavt`,`loaidau` FROM `thietbi_iso` where loaidau !=''";
						                 ...
						                 while($row = mysql_fetch_array($in_result))
								 {
									 $mavattu =$row['mavt'];	 
									 $loaidau =$row['loaidau'];
		$in_sql1="SELECT `somay` FROM `hososcbd_iso` where mavt='$mavattu' GROUP BY mavt,somay";
						                 $in_result1=mysql_query($in_sql1);
						                 while($row = mysql_fetch_array($in_result1))
								 {
									 $sm =$row['somay'];
									 $max =$row['ngaybdtt'];
		$sql="SELECT `stt`,`mavt`,`somay`, date_format(`ngaykt`,'%d-%m-%Y') as ngaykt, max(ngaybdtt) as ngaybdtt,left(honghoc,2) as loaidau  FROM `hososcbd_iso` where mavt='$mavattu' and somay='$sm' $month_string $where_search GROUP BY mavt,somay";
					...
					if (in_array($key, $this->fields_in_list_view)) {
						...
						if ($key == $this->primary_key) {
						    ...
						    $this_row .= "<td >$stt</td>";
							 ...
							 elseif ($key=="ngaybdtt"){
								$red_date = strtotime ( '-15 day' , strtotime($value)) ;
								$red_date = date ( 'd-m-Y' , $red_date );

								$yellow_date = strtotime ( '-1 month' , strtotime($value)) ;
								$yellow_date = date ( 'd-m-Y' , $yellow_date );

								if(strtotime($today) > strtotime($red_date)){
									$bkcanhbao = "#ff9a9a";
								}elseif(strtotime($today)>strtotime($yellow_date)&&strtotime($today)<strtotime($red_date)){
									$bkcanhbao = "#FFFF00";
								}elseif(strtotime($today)<strtotime($yellow_date)){
									$bkcanhbao = "#00BFFF";
								}
                                                                $ngaybdtt=date("d-m-Y", strtotime($value));
								$this_row .= "<td> ".  $ngaybdtt  ." </td>";
							 }elseif($key=="loaidau"){
								$this_row .= "<td> ".  $loaidau  ." </td>";
