...
class MySQLtabledit {
	...
	####################
        ...
        $sql="SELECT `stt`,`tenthietbi`,`tenvt`,`chusohuu`,`serialnumber`,`thly`,date_format(`ngaykd`,'%d-%m-%Y') as ngaykd,date_format(`ngaykdtt`,'%d-%m-%Y') as ngaykdtt,`tlkt`,`hosomay`  FROM `thietbihotro_iso` $where_search $order_by";
		...
		if (mysql_num_rows($result)>0) {
			...
			while ($rij = mysql_fetch_assoc($result)) {
				...
				foreach ($rij AS $key => $value) {
				    ...
				    if($key=="ngaykd") if ($value=="00-00-0000") $value="";
				    if($key=="ngaykdtt") if ($value=="00-00-0000") $value="";
				    if (($key=="tlkt")||($key=="hosomay")) 
				    ...
				    elseif ($key=="ngaykdtt")
					    $head .= "<td NOWRAP style='border:0px solid white;border-bottom:1px solid black'><a href='$this->url_script?$query_sort' class='mte_head'>$show_key</a> $sort_image</td>";
				    elseif ($key=="thly")
						...
						if ($key == $this->primary_key) {
							...
							if($key=="tenthietbi") $mavt=$value;
							if($key=="ngaykd") $ngaykd=$value;
						//	if($key=="hankd") $hankd=$value;
							if($key=="serialnumber") $somay=$value;
							if($key=="hosomay"){
							    ...
							    if($value=="1") 
							    $this_row .= "<td style='background:#DC143C;border:0px solid white;border-bottom:1px solid black'> <span style=\"color:blue;\"><div class=\"noprint\">$value</div></span> </td>";
                                                            else
							    $this_row .= "<td style='border:0px solid white;border-bottom:1px solid black'><span style=\"color:blue;\"><div class=\"noprint\">$value</div></span></a></td>";	 
			...
			if ($this->show_text[$option]) {$show_option = $this->show_text[$option];}
               ...
               if ($option == $in_search_field) {
					...
					if (($option=="stt")||($option=="tlkt")||($option=="hosomay")){}else {
						if ($option=="cdung") $show_option="Danh mục các thiết bị chuyên dụng";
	...
	##################
		...
		$in_id = $_GET['id'];
        $in_sql=mysql_query("SELECT `tenthietbi`,`serialnumber` FROM `thietbihotro_iso` WHERE `stt`='$in_id'");
	...
	###################
		...
		/// STT
			$recc=0;
		$r3=mysql_query("SELECT max(stt) as tt FROM thietbihotro_iso");
...
//////		
	    ...
	    if ($show_key=="Hồ sơ máy") {
            ...
            if($show_key=="Hạn KD/BD")
	    { 
		   if ($value!="") $hankd=$value;
		   else $hankd=12;
		   $field = "<input type='text' name='$key' value='$hankd' maxlength='{$matches[1]}' $style $readonly $field_id>";  
                } 
            if($show_key=="Check nếu là thiết bị chuyên dụng của Xưởng")
	    {  
		    if ($value=='1')
			    $field = "Có:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='radio' name='$key' value='1'  checked> Không:<input type='radio' name='$key' value='0' >"; 
		    else
			    $field = "Có:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type='radio' name='$key' value='1'> Không:<input type='radio' name='$key' value='0' checked>"; 
		    
            } 
	    if($show_key=="Ngày KD/BD")

	    { 
		    if (($value=="")||($value=="0000-00-00")) $value="0000-00-00";
		    else $value = date("d/m/Y",strtotime("$value"));  
		    $field = "<input name='ngaykd'  type='text' value=\"$value\">
				       <A HREF=\"#\" onClick=\"cal.select(document.forms['example'].ngaykd,'anchor1','dd/MM/yyyy'); return false;\"
		...
		$this->content = "
				...
				<div style='width: $this->width_editor;background:#454545'>
					...
					<table cellspacing=0 cellpadding=0 style='border: 0px solid white'>
						<tr>
						<td>
							<button onclick='window.location=\"danhmuchotro.php\";' style='margin: 20px 15px 25px 15px; border: 1px solid #000;'>{$this->text['Go_back']}</button></td>
						<td>
	...
	###################
        ...
        foreach($_POST AS $key => $value) {
            ...
            if($key=="serialnumber"){
                ...
                $in_serialnumber=$value;        // GIU LAI TEN THIET BI DE THONG BAO SAU KHI UPDATE HOAC EDIT              
            }
            if($key=="ngaykd")
            {
		...
		$ngaykd=$value;
            ...
            if($key=="ngaykdtt")
	    {
		    	$value = strtotime(date('d-m-Y', strtotime($ngaykd)) . " +$hankd month");
		...
		$check=0;
		    $result=mysql_query("SELECT * FROM `$this->table`");
            		while ($rij = mysql_fetch_array($result))
			{
				if($temp== $rij['tenthietbi']&&$in_serialnumber==$rij['serialnumber'])	
						$check=1;
			} 
		...
		# edit record:
			...
			$sql = "UPDATE `$this->table` SET $updates WHERE $where LIMIT 1; ";
                ...
                if (mysql_query($sql)) {
        			...
        			$_SESSION['content_saved'] = "
        				<div style='width: $this->width_editor'>
        					<div style='padding: 10px; color:#fff; background: #67B915; font-weight: bold'>{$this->text['saved']} </br> Thiết bị $temp </br> Số Serial Number $in_serialnumber </div>
        				</div>
        				";
        			if ($in_mte_new_rec) {
