...
class MySQLtabledit {
	...
	####################
       ...
       # message after add or edit
		...
		// build query_string
		// query_joomla_component (joomla) 
		$sql="SELECT `stt`,`somay`,`mavt`,max(ngaykt) as ngaykt,max(ngaybdtt) as ngaybdtt,left(ngaykt,2) as ngaycl,`madv`  FROM `hososcbd_iso` where mavt='$mavattu' and somay='$sm' and ngaybdtt='$max' and madv='CNC' $month_string $where_search GROUP BY mavt,somay";
...
*/
		...
		$seach_form .= "
				...
				<td style='padding: 15px; text-align: right; width: $this->width_editor'>
						...
						<option value='CNC'>Đội CNC</option>
						";
