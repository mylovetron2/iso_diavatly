...
class MySQLtabledit {
	...
	####################
		...
		$sql="SELECT `stt`, `username`,`email`,`madv`,`nhom` FROM `users` $where_search $order_by";
					...
					if (in_array($key, $this->fields_in_list_view)) {
						...
						if ($key == $this->primary_key) {
							$buttons = "<td NOWRAP><a href='javascript:void(0)' onclick='del_confirm($value)' title='Delete {$this->show_text[$key]} $value'><IMG SRC='$this->url_base/images/del.png' WIDTH=16 HEIGHT=16 BORDER=0 ALT=''></a>
                                        &nbsp;<a href='?$query_string&mte_a=edit&id=$value' title='Edit {$this->show_text[$key]} $value'><IMG SRC='$this->url_base/images/edit.png' WIDTH=16 HEIGHT=16 BORDER=0 ALT=''></a></td>";
                            $stt= $value;
							...
							if($key=="mavt") $mavt=$value;
						 	...
						 	if($key=="tlkt"){
								...
								$this_row .="<td><a href='TLKT/$mavt'  title='Download' ><span style=\"color:blue;\"> Download</span></a></td>";
							 }
							else
		...
		$seach_form .= "
				...
				<td style='padding: 15px; text-align: right; width: $this->width_editor'>
					<button onclick='window.location=\"$this->url_script?$query_string&mte_a=new\"' style='margin: 0 0 15px 15px; border: 1px solid #000;'>Thêm mới</button>
				</td>
	...
	##################
		...
		$in_id = $_GET['id'];
        $in_sql=mysql_query("SELECT `username`,`nhom` FROM `users` WHERE `stt`='$in_id'");
	...
	###################
		...
		/// STT
			$recc=0;
		$r3=mysql_query("SELECT max(stt) as tt FROM users");
...
//////		
		...
		foreach ($rij AS $key => $value) {
            ...
            if($show_key=="Nhóm"){
                $in_sql="SELECT DISTINCT `nhom` FROM `users`";
                $in_result=mysql_query($in_sql);
               
                while($row = mysql_fetch_array($in_result))
                    {
                        if($row[nhom]==$value)
                            $options .= "<option selected value='$row[nhom]'>$row[nhom]</option>"; 
                        else
                            $options .= "<option value='$row[nhom]'>$row[nhom]</option>"; 
	    ...
	    if ($show_key=="Tài Liệu Kỹ Thuật") {
		    //$field="<a href='upload.php?mavt=$mavt' target=\"_blank\" name='tlkt' ><span style=\"color:blue;\">Upload</span></a>";
		...
		$this->content = "
				...
				<div style='width: $this->width_editor;background:#454545'>
					...
					<table cellspacing=0 cellpadding=0 style='border: 0px solid white'>
						<tr>
						<td>
							<button onclick='window.location=\"quanlyuser.php\";' style='margin: 20px 15px 25px 15px; border: 1px solid #000;'>{$this->text['Go_back']}</button></td>
						<td>
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	###################
        ...
        foreach($_POST AS $key => $value) {
            ...
            if ($key=="username")
            {
                $username=$value;               // GIU LAI MASOMAY DE THONG BAO SAU KHI UPDATE HOAC EDIT
            }
		...
		# edit record:
			...
			$sql = "UPDATE `$this->table` SET $updates WHERE $where LIMIT 1; ";	
                ...
                // THEM VAO BANG danhmucthietbihc    
                if (mysql_query($sql)) {
        			...
        			$_SESSION['content_saved'] = "
        				<div style='width: $this->width_editor'>
        					<div style='padding: 10px; color:#fff; background: #67B915; font-weight: bold'>{$this->text['saved']} </br> USERNAME $username</div>
        				</div>
        				";
        			if ($in_mte_new_rec) {
