...
class MySQLtabledit {
	...
	####################
       ...
       # message after add or edit
		...
		$query_string .= '&s=' . $_GET['s']  . '&f=' . $_GET['f'] . '&n=' . $_GET['n'] .'&y=' . $_GET['y']  . '&v=' . $_GET['v'] ;
		# search
		if ($_GET['s'] && $_GET['f']) {
			...
			if ($in_search_field == $this->primary_key) {
				...
				$where_search = "WHERE $in_search_field LIKE '%$in_search%' and cdung='1'";
			}
		}
		if ($_GET['v'] && $_GET['y']){
			$v =  $_GET['v'];
			...
			if($where_search!=""){
				...
				if($v>=1 &&$v<=12){
					$month_string  = "WHERE ngaykdtt BETWEEN '$y-$v-01 00:00:00' AND '$y-$v-31 00:00:00' and cdung='1'";
				}else if($v=="I"){
					$month_string  = "WHERE ngaykdtt BETWEEN '$y-01-01 00:00:00' AND '$y-03-31 00:00:00' and cdung='1'";
				}else if($v=="II"){
					$month_string  = "WHERE ngaykdtt BETWEEN '$y-04-01 00:00:00' AND '$y-06-31 00:00:00' and cdung='1'";
				}else if($v=="III"){
					$month_string  = "WHERE ngaykdtt BETWEEN '$y-07-01 00:00:00' AND '$y-09-31 00:00:00' and cdung='1'";
				}else if($v=="IV"){
					$month_string  = "WHERE ngaykdtt BETWEEN '$y-10-01 00:00:00' AND '$y-12-31 00:00:00' and cdung='1''";
				}else if($v=="nam"){
					$month_string  = "WHERE ngaykdtt BETWEEN '$y-1-01 00:00:00' AND '$y-12-31 00:00:00' and cdung='1'";
				}
			}
		}else{	
			$m = date('m');
			...
			if($where_search!=""){
				...
				$month_string = "WHERE ngaykdtt BETWEEN '$y-$m-01 00:00:00' AND '$y-$m-31 00:00:00' and cdung='1'";
		}
		$sql="SELECT `stt`,`tenthietbi`,`tenvt`,`chusohuu`,`serialnumber`,date_format(`ngaykd`,'%d-%m-%Y') as ngaykd,max(ngaykdtt) as ngaykdtt,left(ngaykd,2) as ngaycl FROM `thietbihotro_iso` $month_string $where_search  GROUP BY tenthietbi,serialnumber";
...
*/
		...
		foreach ($this->fields_in_list_view AS $option) {
            ...
            if ($option == $in_search_field) {
					...
					if (($option=="stt")||($option=="hankd")||($option=="hosomay")||($option=="cdung")||($option=="tlkt")||($option=="tenvt")||($option=="ngaycl")){}else 
		...
		elseif ($v=="nam") $ten="Năm";else $ten="Tháng";
		$y=$_GET['y'];
	        if ($y=="") 	
			$cury=date("Y");
		else    $cury=$y;
