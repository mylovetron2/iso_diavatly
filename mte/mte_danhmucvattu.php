...
class MySQLtabledit {
	...
	####################
       ...
       # message after add or edit
		...
		// build query_string
		// query_joomla_component (joomla) 
		$sql="SELECT `stt`,`mavattu`,`mota`,`serialnumber`,`dvt` ,soluong,date_format(`ngayth`,'%d-%m-%Y') as ngayth FROM `danhmucvattu_iso`  $month_string $where_search $order_by  ";
				...
				foreach ($rij AS $key => $value) {
				    ...
				    $head .= "<td NOWRAP><a href='$this->url_script?$query_sort' class='mte_head'>$show_key</a> $sort_image</td>";
						...
						if ($key == $this->primary_key) {
							...
							if($key=="mavattu"){
								$this_row .="<td><a href='formsc.php?readmavattu=$value'  title='Hosomay' ><span style=\"color:blue;\">$value</span></a></td>";                         }else
		...
		$this->content = "
				...
				<div style='width: $this->width_editor;background:#454545'>
					...
					<table cellspacing=0 cellpadding=0 style='border: 0px solid white'>
						<tr>
						<td>
							<button onclick='window.location=\"{$_SESSION['hist_page_danhmucvattu']}\";' style='margin: 20px 15px 25px 15px; border: 1px solid #000;'>{$this->text['Go_back']}</button></td>
						<td>
