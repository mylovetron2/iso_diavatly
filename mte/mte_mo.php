...
class MySQLtabledit {
	...
	####################
        ...
        $sql="SELECT `stt`,`mamo`,`tenmo` FROM `mo_iso` $where_search $order_by";
	...
	##################
		...
		$in_id = $_GET['id'];
        $in_sql=mysql_query("SELECT `mamo`,`tenmo` FROM `mo_iso` WHERE `stt`='$in_id'");
		...
		$this->content = "
				...
				<div style='width: $this->width_editor;background:#454545'>
					...
					<table cellspacing=0 cellpadding=0 style='border: 0px solid white'>
						<tr>
						<td>
							<button onclick='window.location=\"mo_iso.php\";' style='margin: 20px 15px 25px 15px; border: 1px solid #000;'>{$this->text['Go_back']}</button></td>
						<td>
