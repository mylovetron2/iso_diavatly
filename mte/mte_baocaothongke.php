	...
	####################
       ...
       # message after add or edit
		...
		if ($_GET['s'] && $_GET['f']) {
			...
			$mamay = explode("-",$in_search);
			if ( $in_search_field =="mamay"	)
			$where_search = "and mavt LIKE '%$mamay[0]%' and somay  LIKE '%$mamay[1]%' ";
			else 
						...
						if ($key == $this->primary_key) {
							...
							$this_row .= "<td >$stt</td>";
							 ...
							 elseif($key=="bangsl"){
								 ...
								 $this_row .="<td><a href='formsc.php?readhoso=$hoso'  title='Hosomay' ><span style=\"color:blue;\">$hoso</span></a></td>";                           }else if($key=="vitrimaybd"){
		...
		foreach ($this->fields_in_list_view AS $option) {
		    ...
		    if ($option == $in_search_field) {
		    			...
		    			if (($option=="stt")||($option=="bangsl")||($option=="ngayth")||($option=="ngaykt")||($option=="ngaysc")||($option=="phieuktsc")||($option=="tenvt")){}else 
		...
		/*$seach_form .= "
				...
				<td style='padding: 15px; width: $this->width_editor'>
					...
					</form></td><td style='padding: 15px;'><input  style='width:120px;color:red;border: 1px solid #000;height:20px' type=\"submit\" value='In ấn và xuất file' onClick=\"window.location.href='report/view1rpt.php'\" /></td>
			</tr>
			</table>
			</div>
			";
		...
		$seach_form .= "
				...
				<td style='padding: 15px; width: $this->width_editor'>
					...
					<A HREF=\"#\" onClick=\"cal.select(document.forms['example'].to,'anchor1','dd/MM/yyyy'); return false;\"
					NAME=\"anchor1\" ID=\"anchor1\"><img src=\"upload/calendar.gif\" ></A>
					<input name ='submit' type='submit' value='Tìm kiếm' style='width:90px; border: 1px solid #000'>
					<input name ='submit' type='submit' value='All' style='width:40px; border: 1px solid #000'>
					</form></td>
					<td style='padding: 15px;'><input  style='width:60px;color:red;border: 1px solid #000;height:20px' type=\"submit\" value='Mau 01' onClick=\"window.location.href='./baocaothang01.php?from=$ngayfrom&to=$ngayto&s=$s&f=$f'\" /></td>
					<td style='padding: 15px;'><input  style='width:60px;color:red;border: 1px solid #000;height:20px' type=\"submit\" value='Mau 02' onClick=\"window.location.href='./baocaothang02.php?from=$ngayfrom&to=$ngayto&s=$s&f=$f'\" /></td>
					<td style='padding: 15px;'><input  style='width:60px;color:red;border: 1px solid #000;height:20px' type=\"submit\" value='Mẫu cũ' onClick=\"window.location.href='./baocaothang.php?from=$ngayfrom&to=$ngayto&s=$s&f=$f'\" /></td>
			</tr>
			</table>
			</div>
			";
