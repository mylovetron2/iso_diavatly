...
class MySQLtabledit {
	...
	####################
       ...
       # message after add or edit
		...
		if (($in_search_field=="solanhong")&&($in_search!="")){
			$r3 = mysql_query("SELECT DISTINCT mavt,somay FROM hososcbd_iso");
			$flag=1;
                $stt=0; 
		while($row = mysql_fetch_array($r3))
		{
			$mavattu =$row['mavt'];
			$somay = $row['somay'];
			$sql="SELECT `stt`,`mavt`,`somay`, `hoso`,date_format(`ngaykt`,'%d-%m-%Y') as ngaykt, `honghoc`, `khacphuc` ,`ttktafter`,count(*) as solanhong,`nhomsc`,left(ngaykt,2) as phieuktsc,left(ngaykt,2) as bangsl  FROM `hososcbd_iso` $month_string and mavt='$mavattu' and somay='$somay' and ttktafter !='Hỏng' and ngaykt !='00-00-0000' $order_by";
		...
		if (mysql_num_rows($result)>0) {
			...
			while ($rij = mysql_fetch_assoc($result)) {
				...
				$solanhong = $rij['solanhong'];
				
			if($solanhong>=$in_search) {	
					...
					if (in_array($key, $this->fields_in_list_view)) {
						if ($count == 1) {
							...
							if ($flag==1) {
							    ...
							    if (($key=="bangsl")||($key=="nhomsc")||($key=="phieuktsc")) 
								    $head .= "<td NOWRAP style='border:0px solid white'><a href='$this->url_script?$query_sort' class='mte_head'><div class=\"noprint\">$show_key</div></a> $sort_image</td>";
							    else
						...
						$this_row .= "<td >$stt</td>";
							 ...
							 elseif($key=="bangsl"){
								...
								$this_row .="<td><a href='download.php?hoso=$hoso'  title='Download' ><span style=\"color:blue;\"> <div class=\"noprint\">Download</div></span></a></td>";
							 }else if($key=="hoso"){
								$this_row .="<td><a href='formsc.php?readhoso=$hoso'  title='Hosomay' ><span style=\"color:blue;\">$hoso</span></a></td>";
							 }elseif ($key=="phieuktsc") { 
								    $this_row .="<td style='border:0px solid white;border-bottom:1px solid black'><a  href='formsc.php?phieuktsc=$hoso'  title='Phieu KTSC' ><span style=\"color:blue;\"><div class=\"noprint\">Phiếu KTSC</div></span></a></td>";				                                                    }else if($key=="nhomsc"){
								 $this_row .="<td style='border:0px solid white;border-bottom:1px solid black'><div class=\"noprint\"><a href='thongkegiolv.php?f=mahoso&s=$hoso&v=all&y=all'  title='Hosomay' ><span style=\"color:blue;\">$value</span></a></div></td>";} 
							 else	
			...
			$sql="SELECT `stt`,`mavt`,`somay`, `hoso`,date_format(`ngaykt`,'%d-%m-%Y') as ngaykt, `honghoc`, `khacphuc` ,`ttktafter`,`nhomsc`,left(ngaykt,2) as phieuktsc,left(ngaykt,2) as bangsl  FROM `hososcbd_iso`  $month_string $where_search $order_by";
						...
						if ($count == 1) {
                            ...
                            if($key==$this->primary_key){
                                $head .= "<td visibility:collapse><span style=\"font-weight:bold;\">STT</span></td>";
                                   ...
                                   if (($key=="bangsl")||($key=="nhomsc")||($key=="phieuktsc")) 
					$head .= "<td NOWRAP style='border:0px solid white'><a href='$this->url_script?$query_sort' class='mte_head'><div class=\"noprint\">$show_key</div></a> $sort_image</td>";
				else if ($key=="ttktafter") $head .= "<td NOWRAP style='border:0px solid white'><a href='$this->url_script?$query_sort' class='mte_head'>$show_key</a> $sort_image</td>";
				else $head .= "<td NOWRAP><a href='$this->url_script?$query_sort' class='mte_head'>$show_key</a> $sort_image</td>";
			    }
						}
						if ($key == $this->primary_key) {
							$stt++;
							$this_row .= "<td >$stt</td>";
							 ...
							 if($key=="ttktafter")  {
							    if($value=="Hỏng") 
							    $this_row .= "<td style='background:#DC143C;border:0px solid white;border-bottom:1px solid black'>" . substr(strip_tags($value), 0, 300) . '</td>';
                                                            else
							    $this_row .= "<td style='border:0px solid white;border-bottom:1px solid black'>" . substr(strip_tags($value), 0, 300) . '</td>';	 
							 }elseif($key=="mavt"){
								$mavt=$value;
								$this_row .="<td><a  href='download.php?mavttlkt=$mavt'  title='TLKT' ><span style=\"color:blue;\">$mavt</span></a></td>";	
							 }
							 ...
							 elseif($key=="tenvt"){
								$r5 = mysql_query("SELECT DISTINCT tenvt FROM thietbi_iso where mavt='$mavt'");
		...
		foreach ($this->fields_in_list_view AS $option) {
			...
			if ($option == $in_search_field) {
				...
				if (($option=="stt")||($option=="ngaykt")||($option=="bangsl")||($option=="phieuktsc")){}else 
		...
		$seach_form .= "
				...
				<td style='padding: 15px; width: $this->width_editor'>
					...
					</form></td><td style='padding: 15px;'><input  style='width:120px;color:red;border: 1px solid #000;height:20px' type=\"submit\" value='In ấn và xuất file' onClick=\"window.location.href='./inanvaxuatfile.php?from=$ngayfrom&to=$ngayto&s=$s&f=$f'\" /></td>
			</tr>
			</table>
			</div>
		";
