       ...
       # message after add or edit
		...
		if ($_GET['v'] && $_GET['y'] && $_GET['dv']){
			...
			if($where_search!=""){
				...
				if($v>=1 &&$v<=12){
					$month_string  = "AND ngaybdtt BETWEEN '$y-$v-01 00:00:00' AND '$y-$v-31 00:00:00'  and ttktafter !='Hỏng'";
				}else if($v=="I"){
					$month_string  = "AND ngaybdtt BETWEEN '$y-01-01 00:00:00' AND '$y-03-31 00:00:00'  and ttktafter !='Hỏng'";
				}else if($v=="II"){
					$month_string  = "AND ngaybdtt BETWEEN '$y-04-01 00:00:00' AND '$y-06-31 00:00:00'  and ttktafter !='Hỏng'";
				}else if($v=="III"){
					$month_string  = "AND ngaybdtt BETWEEN '$y-07-01 00:00:00' AND '$y-09-31 00:00:00'  and ttktafter !='Hỏng'";
				}else if($v=="IV"){
					$month_string  = "AND ngaybdtt BETWEEN '$y-10-01 00:00:00' AND '$y-12-31 00:00:00'  and ttktafter !='Hỏng'";
				}else if($v=="nam"){
					$month_string  = "AND ngaybdtt BETWEEN '$y-1-01 00:00:00' AND '$y-12-31 00:00:00'  and ttktafter !='Hỏng'";
				}
			}
		}else{	
			$m = date('m');
		...
		// build query_string
		// query_joomla_component (joomla) 
		$sql="SELECT `stt`,`somay`,`mavt`,max(ngaykt) as ngaykt,max(ngaybdtt) as ngaybdtt,left(ngaykt,2) as ngaycl,`madv`  FROM `hososcbd_iso` where mavt='$mavattu' and somay='$sm' and ngaybdtt='$max' and madv='KTKT' $month_string $where_search GROUP BY mavt,somay";
...
*/
		...
		$seach_form .= "
				...
				<td style='padding: 15px; text-align: right; width: $this->width_editor'>
						...
						<option value='KTKT'>Đội KTKT</option>
						";
