	...
	####################
       ...
       # message after add or edit
		...
		if ($_GET['from'] && $_GET['to']){
			$from = $_GET['from'];
			$to = $_GET['to'];
			if(($from=="all")||($to=="all")) $month_string="WHERE ngayth LIKE'%%'";else {
		...
		if (($in_search_field=="giolvt")&&($in_search=="all")){
                                $head .= "<td visibility:collapse><span style=\"font-weight:bold;\">STT</span></td>";
				$head .= "<td NOWRAP><a href='' class='mte_head'>Họ tên</a></td>";
				$head .= "<td NOWRAP><a href='' class='mte_head'>Giờ công làm việc</br> trong tháng</a></td>";
			$donvi="chuẩn chỉnh máy địa vật lý";
			$sqlng=mysql_query("SELECT hoten FROM resume where donvi like'%$donvi%' and nghiviec !='yes'") ;
	        $stt=0;	
			while($row = mysql_fetch_array($sqlng))
			{
                        $hoten=$row['hoten']; 
        	$sql="SELECT `stt`,`hoten`, sum(giolv) as giolvt FROM `ngthuchien_iso` $month_string and hoten='$hoten' ";
		...
		if (mysql_num_rows($result)>0) {
			...
			while ($rij = mysql_fetch_assoc($result)) {
				...
				$head .= "<td NOWRAP><a href='$this->url_script?$query_sort' class='mte_head'>$show_key</a> $sort_image</td>";
						...
						/*	 if($key=="ngayth")  {
								 $ngayth = $value;
								 if($ngayth=="00-00-0000") $ngaykt="";
								 $this_row .= '<td>' . substr(strip_tags($ngayth), 0, 300) . '</td>';
							 }
							 elseif($key=="ngaykt")  {
							    $ngaykt =  $value;
							    if($ngaykt=="00-00-0000") $ngaykt="";
							    $this_row .= '<td>' . substr(strip_tags($ngaykt), 0, 300) . '</td>';
							 }else if($key=="mahoso"){
								 $hoso= $value;
								 $this_row .="<td><a href='formsc.php?readhoso=$hoso'  title='Hosomay' ><span style=\"color:blue;\">$hoso</span></a></td>";                           }elseif($key=="mamay"){
								$mavt=$value;
        ...
        $sql="SELECT `stt`,`hoten`,`mahoso`,`mamay`,`somay`,`giolv`,date_format(`ngayth`,'%d-%m-%Y') as ngayth,date_format(`ngaykt`,'%d-%m-%Y') as ngaykt FROM `ngthuchien_iso` $month_string $where_search or ngaykt='0000-00-00' and ngayth!='0000-00-00' $where_search $order_by";
		$result = mysql_query($sql);
		# navigation 2/3
		$hits_total = mysql_num_rows($result); 
		$sql .= " LIMIT $start, $this->num_rows_list_view";
							...
							$this_row .= "<td >$stt</td>";
							 ...
							 elseif($key=="ngaykt")  {
								...
								$this_row .="<td><a  href='download.php?mavttlkt=$mavt'  title='TLKT' ><span style=\"color:blue;\">$mavt</span></a></td>";	
								 ...
								 if ($key=="giolv") {
										$ngthsql = mysql_query("SELECT giolv1,giolv2,giolv3,giolv4,giolv5,giolv6,giolv7,giolv8,giolv9,giolv10,giolv11,giolv12 FROM ngthuchien_iso WHERE hoten='$in_search' and mahoso='$hoso' and ngaykt BETWEEN '$yfrom-$mfrom-$dfrom 00:00:00' AND '$yto-$mto-$dto 00:00:00' or hoten='$in_search' and mahoso='$hoso' and ngaykt='0000-00-00' and ngayth!='0000-00-00'  ") ;
		...
		$giolv12 = $row['giolv12'];
			...
			if($giolv<0) $giolv=0; 
			$sumgiolv=$sumgiolv + $giolv;
										}
										$this_row .= '<td>' . substr(strip_tags($giolv), 0, 300) . '</td>';
                     } else 			
								$this_row .= '<td>' . substr(strip_tags($value), 0, 300) . '</td>';
							 if ($in_search_field=="hoten") {
									if ($key=="giolv") {
									
										$this_row1= "<td>$sumgiolv</td>";
									}
								}
						}
					}
			...
			if ($in_search_field=="hoten") 
				$rows .= "<tr style='background:$background'><td></td><td>Tổng số giờ làm việc</td><td></td><td></td><td></td>$buttons $this_row1<td></td><td></td></tr>";
		}else {
			$head = "<td style='padding:50px'>{$this->text['Nothing_found']}...</td>";
		}
		}
		...
		foreach ($this->fields_in_list_view AS $option) {
		    ...
		    if ($option == $in_search_field) {
		    			...
		    			if (($option=="stt")||($option=="bangsl")||($option=="giolv")){}else 
